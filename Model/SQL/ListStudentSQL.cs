...
namespace SistemaRefeitorio.Model.SQL
{
    public class ListStudentSQL : Connection
    {
        ...
        public void GetDataByName(DataGridView dgListStudent, string nameStudent)
        {
            try
            {
                Connect();

                string selectName = "SELECT raStudent, name, email, cpf, coffeCredit, lunchCredit, " +
                                "dinnerCredit, telephone FROM students WHERE name LIKE @name";

                MySqlCommand cmd = new MySqlCommand(selectName, SqlConnection);

                cmd.Prepare();

                cmd.Parameters.AddWithValue("@name", String.Format("%{0}%", nameStudent));
